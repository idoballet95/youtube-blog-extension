...
- **반드시 최소 3개 이상의 카드를 만드세요.** 각 섹션마다 1개 이상 배치하는 것을 기본으로 하고, 본문 전체에서 시각화 가능한 데이터(수치/비교/순위/시간순 흐름/포메이션)를 최대한 찾아내세요.
- 한 섹션에 시각화할 데이터가 여러 개면 그 섹션에 2개를 배치해도 됩니다. 어떻게든 전체 3개 이상을 채우세요.
- **타입 다양화**: highlight만 반복하지 말고 서로 다른 타입을 섞으세요. 두 대상(선수/팀/경기·전후 비교)을 견줄 수 있으면 compare, 시간 흐름·경기 전개면 timeline, 여러 항목 순위·명단이면 ranking, 라인업이 명확하면 formation을 우선 고려하세요. highlight는 단일 대상의 핵심 수치를 강조할 때 쓰세요.
